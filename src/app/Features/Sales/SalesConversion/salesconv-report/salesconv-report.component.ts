...
@Component({
  selector: 'app-salesconv-report',
  templateUrl: './salesconv-report.component.html',
  styleUrls: ['./salesconv-report.component.scss'],
})
export class SalesconvReportComponent implements OnInit {
  ...
  pushvalue(val) {
    if (val.state == false) {
      if (this.selectedDataGrouping.length >= 3) {
        alert('Select up to 3 filters for your group');
      } else {
        val.state = true;
